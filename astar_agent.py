...
def get_successors(current_state):  # more like surrounding grid positions
    ...
    if not left_state.is_terminated:
        successors.append((left_state, "LEFT", euclideanCost(current_state)))
    right_state = SnakeState.get_next(current_state, "RIGHT")
    if not right_state.is_terminated:
        successors.append((right_state, "RIGHT", euclideanCost(current_state)))
    up_state = SnakeState.get_next(current_state, "UP")
    if not up_state.is_terminated:
        successors.append((up_state, "UP", euclideanCost(current_state)))
    down_state = SnakeState.get_next(current_state, "DOWN")
    if not down_state.is_terminated:
        successors.append((down_state, "DOWN", euclideanCost(current_state)))
    return successors


def aStar_search(snake, heuristic):
    ...
    while 1:
        ...
        if current_state.body[0] not in visited:
            ...
            for childNode, direction, cost in get_successors(current_state):
